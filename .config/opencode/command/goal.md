---
description: Pursue a long-running objective using the goal skill
---

Load the `goal` skill and pursue this objective as a long-running goal:

$ARGUMENTS

If no objective is provided, ask for the goal in one short question.

Apply the skill's completion audit before claiming the goal is complete. If the goal cannot be completed in this turn, make concrete progress, summarize what changed, and state the next best action.
